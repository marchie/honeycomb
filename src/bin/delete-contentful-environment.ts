...
(async () => {
  const options = yargs
    ...
    .option("environmentId", {
      describe: "Name of the Contentful environment to delete",
      type: "string",
      demandOption: true,
    }).argv;
  ...
  try {
    ...
    const deletedEnvironmentName = await migrator.DeleteEnvironment({
      environmentId,
    });

    console.log(`Deleted Contentful environment "${deletedEnvironmentName}"`);
  } catch (e) {
    console.error(
      `Error deleting Contentful environment "${environmentId}": ${e}`,
    );
  }
})();
